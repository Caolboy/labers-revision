<template>
  <div class="p-6 bg-white rounded-2xl shadow-md mt-8 font-sans">
    <div class="flex flex-col sm:flex-row sm:items-center justify-between gap-4 mb-6">
      <h2 class="text-2xl font-semibold text-orange-600">All Labs</h2>
      <input
        v-model="searchQuery"
        @input="fetchLabs"
        type="text"
      ...
      <table class="min-w-full text-sm text-gray-700">
        ...
        <tbody>
          ...
          >
            ...
            <td class="py-3 px-4">{{ lab.name }}</td>
            <td class="py-3 px-4 max-w-xs whitespace-normal break-words text-gray-600">
              {{ lab.description }}
            </td>
            <td class="py-3 px-4 text-center">
              <span
                :class="lab.status === 'active' ? 'bg-green-500' : 'bg-red-500'"
          ...
          <tr v-if="labs.length === 0">
            <td colspan="5" class="p-4 text-center text-gray-500">No labs found.</td>
          </tr>
        </tbody>
      </table>
    </div>
    ...
    <transition name="fade-scale">
      ...
      >
        <div class="bg-white p-6 rounded-xl w-full max-w-md shadow-lg" @click.stop>
          <h3 class="text-xl font-semibold text-orange-600 mb-4 text-center">Edit Lab</h3>
          <form @submit.prevent="updateLab" class="space-y-4">
            <label class="block mb-1 font-semibold">Name</label>
            <input type="text" v-model="editLab.name" placeholder="Name" class="w-full border border-orange-300 rounded-md p-2" />
            <label class="block mb-1 font-semibold">Description</label>
            <textarea v-model="editLab.description" placeholder="Description" class="w-full border border-orange-300 rounded-md p-2 min-h-[100px]"></textarea>
            <label class="block mb-1 font-semibold">Status</label>
            <select v-model="editLab.status" class="w-full border border-orange-300 rounded-md p-2">
            ...
            <div v-else-if="editLab.image" class="mb-4 flex justify-center">
              <img :src="'/' + editLab.image" alt="Existing Preview" class="h-20 rounded object-cover border border-gray-300" />
            </div>
    ...
    <transition name="fade-scale">
      ...
      >
        <div class="bg-white p-6 rounded-xl w-full max-w-sm shadow-lg text-center" @click.stop>
          <h3 class="text-xl font-semibold text-orange-600 mb-3">Delete Lab</h3>
          <p class="text-gray-700 mb-5">Are you sure you want to delete <strong>{{ deleteLab.name }}</strong>?</p>
          <div class="flex justify-center gap-4">
            <button @click="confirmDeleteLab" class="bg-red-500 hover:bg-red-600 text-white px-4 py-2 rounded-md font-semibold">Delete</button>
...
const paginatedLabs = computed(() => {
  ...
  return labs.value.slice(start, start + itemsPerPage);
});

const totalPages = computed(() => Math.ceil(labs.value.length / itemsPerPage));
...
const updateLab = async () => {
  ...
  formData.append("name", editLab.value.name);
  formData.append("description", editLab.value.description);
  formData.append("status", editLab.value.status);
  if (newImage.value) formData.append("image", newImage.value);

  try {
    await axios.post(`/admin/labs/${editLab.value.id}?_method=PUT`, formData, {
...
const confirmDeleteLab = async () => {
  try {
    await axios.delete(`/admin/labs/${deleteLab.value.id}`);
    await fetchLabs();
    closeDeleteModal();
  } catch (error) {
    console.error(error);
