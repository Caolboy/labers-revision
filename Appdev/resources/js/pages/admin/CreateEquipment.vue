<template>
  <div>
    <h2 class="text-2xl font-bold text-orange-600 mb-6 text-center">Create New Equipment</h2>
    <form @submit.prevent="submitForm" enctype="multipart/form-data" class="space-y-5">
      ...
      <div>
        <label for="quantity" class="block text-sm font-semibold mb-1">Quantity <span class="text-red-500">*</span></label>
        <input
          id="quantity"
      ...
      <div>
        ...
        >
          <input
            ...
            @change="onFileChange"
          />
          <p class="text-orange-400 text-sm mb-1">
            Choose or Drop File
          </p>
    ...
    <p v-if="success" class="mt-4 text-center text-green-600 font-semibold">Equipment created successfully!</p>
  </div>
</template>
...
async function submitForm() {
  ...
  try {
    ...
    if (err.response?.data?.message) {
      ...
      error.value = 'An error occurred while creating equipment.'
    }
  } finally {
    loading.value = false
  }
